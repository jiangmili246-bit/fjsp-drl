...
from utils.plot_demo import build_gantt_snapshot, draw_gantt
...
def main():
    ...
    initial_snapshot = build_hetero_graph_snapshot(jobs, active, done, op_ptr, num_m, phase='initial')
    ...
    while len(done)<len(jobs):
        ...
        if after_summary is None and all(j.job_id in active for j in random_arr+urgent_arr):
            ...
            if after_summary['legal actions']==0: after_summary=None
                ...
                running_snapshot = build_hetero_graph_snapshot(jobs, active, done, op_ptr, num_m, phase='disturbance')
    ...
    finished_snapshot = build_hetero_graph_snapshot(jobs, active, set(job_map.keys()), {j.job_id:j.num_ops for j in jobs}, num_m, selected_edges, phase='after')
    if running_snapshot is None:
        running_snapshot = build_hetero_graph_snapshot(jobs, active, done, op_ptr, num_m, phase='disturbance')

    hetero_ok = True
    try:
        draw_hetero_graph(initial_snapshot, 'results/hetero_graph_initial.png', 'Initial State')
        draw_hetero_graph(running_snapshot, 'results/hetero_graph_disturbance.png', 'Concurrent Disturbance')
        draw_hetero_graph(finished_snapshot, 'results/hetero_graph_after.png', 'After Rescheduling')
    except Exception:
        hetero_ok = False

    disturbance_time = min([j.rd for j in random_arr + urgent_arr]) if (random_arr or urgent_arr) else None
    initial_gantt = build_gantt_snapshot([], num_machines=num_m, disturbance_time=None)
    disturbance_records = [r for r in schedule_records if disturbance_time is not None and r[3] < disturbance_time]
    disturbance_gantt = build_gantt_snapshot(disturbance_records, num_machines=num_m, disturbance_time=disturbance_time)
    after_gantt = build_gantt_snapshot(schedule_records, num_machines=num_m, disturbance_time=disturbance_time)

    draw_gantt(initial_gantt, 'results/gantt_initial.png', 'Initial State')
    draw_gantt(disturbance_gantt, 'results/gantt_disturbance.png', 'Concurrent Disturbance')
    draw_gantt(after_gantt, 'results/gantt_after.png', 'After Rescheduling')
    ...
    with open('README_Thesis.md','w',encoding='utf-8') as f:
        ...
        f.write('当前 demo 用于中期检查，证明动态扰动、三元异构图、J-O/O-M 边更新、合法动作集合、结果表格和可视化流程已经跑通。\n\n其中：hetero_graph_initial.png 表示初始状态；hetero_graph_disturbance.png 表示并发扰动触发时状态；hetero_graph_after.png 表示重调度处理后状态。\n\n')
        f.write('运行命令：\n\npython run_small_demo.py\n\n')
        f.write('本地运行后会生成：\n- results/graph_debug.txt\n- results/demo_metrics.csv\n- results/demo_metrics.xlsx\n- results/training_curve.png\n- results/hetero_graph_initial.png\n- results/hetero_graph_disturbance.png\n- results/hetero_graph_after.png\n- results/hetero_graph_before.png (compatibility alias)\n- results/hetero_graph_after.png (compatibility alias)\n- results/gantt_before.png\n- results/gantt_after.png\n\n')
        ...
        if not hetero_ok:
            f.write('图形说明：当前环境缺少 matplotlib，异构图绘制不可用。可安装：pip install matplotlib\n')
