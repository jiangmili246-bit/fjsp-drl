...
def main():
    ...
    while len(done)<len(jobs):
        ...
        if after_summary is None and all(j.job_id in active for j in random_arr+urgent_arr):
            ...
            if after_summary['legal actions']==0: after_summary=None
                ...
                running_snapshot = build_hetero_graph_snapshot(jobs, active, done, op_ptr, num_m, phase='running')
    ...
    finished_snapshot = build_hetero_graph_snapshot(jobs, active, set(job_map.keys()), {j.job_id:j.num_ops for j in jobs}, num_m, selected_edges, phase='finished')
